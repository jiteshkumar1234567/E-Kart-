...
import axios from "axios";
import { useNavigate } from "react-router-dom";
import { BASE_URL } from "../lib/api"; // e.g. "https://e-kart-2-10-2.onrender.com"
...
const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/v1/order/admin/all`, {
  ...
  // 🔹 UPDATE ORDER STATUS (delivered only for now)
  const updateStatus = async (orderId) => {
    try {
      await axios.put(
        `${BASE_URL}/api/v1/order/admin/deliver/${orderId}`,
        {}, // backend expects empty body
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                disabled={order.paymentStatus === "delivered"}
              >
                {order.paymentStatus === "delivered" ? "Delivered" : "Mark Delivered"}
              </button>
            </div>
