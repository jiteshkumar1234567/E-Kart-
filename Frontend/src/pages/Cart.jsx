...
const Cart = () => {
  ...
  const coupons = { SAVE10: 10, TRQ591: 15, MSQ009: 50, MSQ070: 80 };
  const applyPromo = () => {
    const code = promo.toUpperCase();
    if (coupons[code]) {
      const discountAmount = Math.round(cart.totalPrice * (coupons[code] / 100));
      setDiscount(discountAmount);
  ...
  if (loading)
    return <div className="pt-28 text-center text-gray-500 text-lg">Loading your cart...</div>;
  ...
  if (!cart?.items?.length)
    return (
      <div className="pt-28 text-center flex flex-col items-center gap-4">
        ...
        <button
          onClick={() => navigate("/products")}
          className="mt-4 bg-pink-600 hover:bg-pink-500 text-white px-6 py-3 rounded-xl font-semibold"
        >
          Start Shopping
        </button>
      </div>
  ...
  return (
    <div className="pt-28 pb-20 max-w-7xl mx-auto px-4">
      ...
      <div className="flex gap-6 items-start">
        <div className="w-[65%] space-y-4">
          {cart.items.map((item) => (
            <div key={item.productId._id} className="flex gap-4 bg-white p-4 rounded-xl shadow">
              <img
              ...
              <div className="flex-1">
                ...
                <p className="text-pink-600 font-bold mt-1">₹{item.price}</p>
                <div className="flex items-center gap-3 mt-3">
                  <button onClick={() => updateQty(item.productId._id, "decrease")} className="p-1 border rounded">
                    <Minus size={14} />
                  </button>
                  <span>{item.quantity}</span>
                  <button onClick={() => updateQty(item.productId._id, "increase")} className="p-1 border rounded">
                    <Plus size={14} />
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white p-5 rounded-xl shadow border">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 border rounded-lg px-3 py-2 text-sm outline-none"
              />
              <button onClick={applyPromo} className="bg-pink-600 hover:bg-pink-500 text-white px-4 rounded-lg text-sm">
                Apply
              </button>
            </div>
            {discount > 0 && <p className="text-green-600 text-sm mt-2">🎉 You saved ₹{discount}</p>}
          </div>
          {/* PRICE DETAILS */}
          <div className="bg-gradient-to-br from-pink-50 to-white p-6 rounded-2xl shadow-md border">
            <h2 className="font-semibold text-lg mb-4">Price Details</h2>
            ...
            <button onClick={handleCheckout} className="mt-5 w-full bg-pink-600 hover:bg-pink-500 text-white py-3 rounded-xl font-semibold">
              Proceed to Checkout
            </button>
          </div>
        </div>
